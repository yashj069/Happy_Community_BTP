...
let arrayOfScore=[{id:"ss",title:"ff",score:0}];
    ...
    async function getMyInterest(userid) {
        ...
        try {
            ...
            const interest = await getDocs(collection(db,'user',userid,'myInterests'));
...
function compare( a, b ) {
    ...
    async function getMyCommunity(userid) {
        ...
        try {
            ...
            const community = await getDocs(collection(db,'user',userid,'myCommunities'));
                        ...
                        arrayOfScore.map(room=>{
                            ...
                            if(room.score>0)
                            {
                                ...
                                if(x!==-1){
                                    ...
                                    return <div className="suggestionCardsStyle">
                                    <Card room={room} docid={myCommunity[x].docid} isJoin={true} callFunc={callCommunity} desc="you can talk about healthy things here."/>  
                                    </div>
                                }
                                 else{
                                    return <div className="suggestionCardsStyle">
                                    <Card room={room} isJoin={false} callFunc={callCommunity} desc="you can talk about healthy things here."/>  
                                    </div>
